...
const ProfileUpdate = () => {
  ...
  const signatureActionSheetRef = useRef<ActionSheetRef>(null);
  ...
  const getUser = async () => {
    try {
      ...
      setSignature(
        response?.signature ? {path: response?.signature} : undefined,
      );
      setName(response?.name || '');
      setEmail(response?.email || '');
      setMobile(
        response?.contactNumber ? response?.contactNumber?.toString() : '',
      );
      setClinicMobile(
        response?.clinicContactNumber
          ? response?.clinicContactNumber?.toString()
          : '',
      );
      ...
      setGender(
        response?.gender
          ? {
              ...
              value:
                genders?.find(item => item.name === response?.gender)?._id ||
                null,
            }
          : undefined,
      );
      setSpecialization(
        response?.specialization?._id
          ? {
              label: response?.specialization?.name,
              value: response?.specialization?._id,
            }
          : undefined,
      );
      setDepartment(
        response?.department?._id
          ? {
      ...
      setExperience(
        response?.experience !== undefined && response?.experience !== null
          ? response?.experience?.toString()
          : '',
      );
      setBefore(
        response?.bookingBeforeTime !== undefined &&
          response?.bookingBeforeTime !== null
          ? response?.bookingBeforeTime?.toString()
          : '',
      );
      setFee(
        response?.fee !== undefined && response?.fee !== null
          ? response?.fee?.toString()
          : '',
      );
      setOldFee(
        response?.oldFee !== undefined && response?.oldFee !== null
          ? response?.oldFee?.toString()
          : '',
      );
      setSymptoms(Array.isArray(response?.symptoms) ? response?.symptoms : []);
      setStartTime(
        moment(response?.startTime, ['HH:mm', 'HH:mm:ss'], true).isValid()
          ? moment(response?.startTime, ['HH:mm', 'HH:mm:ss'], true).toDate()
          : null,
      );
      setEndTime(
        moment(response?.endTime, ['HH:mm', 'HH:mm:ss'], true).isValid()
          ? moment(response?.endTime, ['HH:mm', 'HH:mm:ss'], true).toDate()
          : null,
      );
    } catch (error) {
      ...
      <View style={{flex: 1, backgroundColor: COLORS.background}}>
        ...
        <View style={{width: '100%', paddingHorizontal: '5%'}}>
          ...
          <CustomInput
            ...
            error={valClinicMobile ? true : false}
            errorMessage={valClinicMobile}
            maxLength={10}
          />
          ...
          <CustomSelection
            ...
            label={'Gender'}
            searchAble={false}
            error={valGender ? true : false}
            errorMessage={valGender}
            setErrorMsg={setValGender}
          />
          ...
          <CustomSelection
            ...
            setDataValue={setSpecialization}
            label={'Specializations'}
            searchAble={true}
            error={valSpecialization ? true : false}
          ...
          <CustomInput
            ...
            error={valClinicAddress ? true : false}
            errorMessage={valClinicAddress}
            autoCapitalize="words"
          />
          ...
          <CustomDateTimeInput
            ...
            error={valStartTime ? true : false}
            errorMessage={valStartTime}
          />
          <CustomDateTimeInput
            label="End Time"
            mode="time"
            ...
            error={valEndTime ? true : false}
            errorMessage={valEndTime}
          />
          <CustomInput
            label="Patient Fee"
            value={fee}
            ...
            error={valFee ? true : false}
            errorMessage={valFee}
            keyboardType="number-pad"
          />
          <CustomInput
            label="Regular Patient Fee"
            value={oldFee}
            placeholder="Enter regular patient fee"
            onChangeText={(e: string) => {
              setOldFee(e);
              setValOldFee('');
            }}
            error={valOldFee ? true : false}
            errorMessage={valOldFee}
            keyboardType="number-pad"
          />
          <CustomInput
            label="Experience"
            ...
            error={valExperience ? true : false}
            errorMessage={valExperience}
            keyboardType="number-pad"
          />
          <CustomInput
            label="Take Appointment Before"
            value={before}
            placeholder="e.g. 1"
            onChangeText={(e: string) => {
              setBefore(e);
            ...
            keyboardType="number-pad"
          />
          <Text style={{color: COLORS.subText}}>Note:- It takes only hours</Text>
          <CustomInput
            label="Symptoms (comma separated)"
            value={symptoms.join(', ')}
            placeholder="Enter symptoms e.g. fever, neck pain"
            onChangeText={(e: string) => {
              const symptomArray = e
                .split(',')
                .map(s => s.trim())
                .filter(s => s.length > 0);
          ...
          <View style={{width: '100%', marginTop: 20}}>
            <View style={styles.imgView}>
              <TouchableOpacity onPress={showSignatureActionSheet}>
                <View style={styles.signatureContainer}>
                  {signature?.path ? (
                    <ProfileImage
                      url={signature?.path}
                      onPress={showSignatureActionSheet}
              ...
              {valSignature && (
                <Text style={styles.errorText}>{valSignature}</Text>
              )}
            </View>
          </View>
        </View>
        ...
        <ActionSheet
          ...
          <View style={styles.cameratOptions}>
            <TouchableOpacity
              ...
              <EvilIcons name="camera" size={40} color={COLORS.text} />
